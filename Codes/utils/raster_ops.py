    ...
    with rio.open(
        ...
        for id, (layer, layer_name) in enumerate(zip(input_files_list, band_key_list), start=1):
            with rio.open(layer) as src:
                ...
                # a check to see if the data is annual or monthly, and setting band name accordingly
                if len(os.path.basename(layer).split('.')[0].split('_')[-1]) == 4:  # check for annual data
                    year = os.path.basename(layer).split('.')[0].split('_')[-1]
                    band_name = f'{layer_name}_{year}'

                elif len(os.path.basename(layer).split('.')[0].split('_')[-1]) <= 2:  # check for monthly data
                    year = os.path.basename(layer).split('.')[0].split('_')[-2]
                    month = os.path.basename(layer).split('.')[0].split('_')[-1]
                    band_name = f'{layer_name}_{year}_{month}'

                elif not os.path.basename(layer).split('.')[0].split('_')[0].isdigit():  # check if static data. basically checking if the last name is a digit or not.
                                                                                         # if not a digit, then the code enters this condition
                    band_name = f'{layer_name}'
